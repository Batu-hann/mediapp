...
export async function cancelAllMedicationReminders(): Promise<void> {
  ...
  try {
    ...
    const reminderKeys = keys.filter((key) => key.startsWith('mediassist_notif_ids:'));
    const metaKeys = keys.filter((key) => key.startsWith(NOTIF_META_PREFIX));

    const storedIds = await AsyncStorage.multiGet(reminderKeys);
    const ids = storedIds.flatMap(([, value]) => {
      if (!value) return [];
      try {
    ...
    const medIds = scheduled
      .map((notification: any) => notification.identifier ?? notification.request?.identifier)
      .filter((id: unknown): id is string => typeof id === 'string' && id.startsWith('med-'));

    for (const id of Array.from(new Set([...ids, ...medIds]))) {
      try { await Notifications.cancelScheduledNotificationAsync(id); } catch {}
    }

    const keysToRemove = [...reminderKeys, ...metaKeys];
