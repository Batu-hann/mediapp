...
if not BASE_URL:
    ...
    fe_path = Path(__file__).resolve().parents[2] / "frontend" / ".env"
    if not fe_path.exists():
        fe_path = Path("/app/frontend/.env")
    fe_env = fe_path.read_text()
...
# ---------- Pharmacy ----------
class TestPharmacy:
    def test_cities_endpoint(self, api_client, auth_headers):
        r = api_client.get(f"{BASE_URL}/api/pharmacies/cities", headers=auth_headers, timeout=10)
        assert r.status_code == 200
        assert "cities" in r.json()
        assert len(r.json()["cities"]) > 0

    def test_districts_endpoint(self, api_client, auth_headers):
        r = api_client.get(f"{BASE_URL}/api/pharmacies/districts?city=istanbul", headers=auth_headers, timeout=10)
        assert r.status_code == 200
    ...
    def test_nearby_default(self, api_client, auth_headers):
        ...
        if r.status_code == 503:
            assert r.json().get("detail") == "Gerçek eczane verisi alınamadı."
        else:
            d = r.json()
    ...
    def test_nearby_oncall_only(self, api_client, auth_headers):
        ...
        if r.status_code == 503:
            ...
            for p in r.json()["pharmacies"]:
                assert (p.get("on_call") is True) or (p.get("isOnDuty") is True)
    ...
    def test_nearby_separated(self, api_client, auth_headers):
        # Duty
        r1 = api_client.get(f"{BASE_URL}/api/pharmacies/duty/nearby?lat=41.0082&lon=28.9784&radius_m=5000",
                            headers=auth_headers, timeout=10)
        assert r1.status_code in (200, 503)
        if r1.status_code == 503:
            assert r1.json().get("detail") == "Gerçek eczane verisi alınamadı."
        else:
            assert "pharmacies" in r1.json()
            for p in r1.json()["pharmacies"]:
                assert (p.get("on_call") is True) or (p.get("isOnDuty") is True)

        # All
        r2 = api_client.get(f"{BASE_URL}/api/pharmacies/all/nearby?lat=41.0082&lon=28.9784&radius_m=5000",
                            headers=auth_headers, timeout=10)
        assert r2.status_code in (200, 503)
        if r2.status_code == 503:
            assert r2.json().get("detail") == "Gerçek eczane verisi alınamadı."
        else:
            assert "pharmacies" in r2.json()

    def test_by_city_separated(self, api_client, auth_headers):
        # Duty
        r1 = api_client.get(f"{BASE_URL}/api/pharmacies/duty/by-city?city=istanbul&district=kadikoy",
                            headers=auth_headers, timeout=10)
        assert r1.status_code in (200, 503)
        if r1.status_code == 503:
            assert r1.json().get("detail") == "Gerçek eczane verisi alınamadı."
        ...
        # All
        r2 = api_client.get(f"{BASE_URL}/api/pharmacies/all/by-city?city=istanbul&district=kadikoy",
                            headers=auth_headers, timeout=10)
        assert r2.status_code in (200, 503)
        if r2.status_code == 503:
            assert r2.json().get("detail") == "Gerçek eczane verisi alınamadı."
...
class TestChat:
    ...
    def test_chat_send_authenticated(self, api_client, auth_headers):
        # Budget may be exceeded - 200, 500, or 502 are acceptable
